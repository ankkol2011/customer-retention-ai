...
from src.monitoring.drift import monitor_batch_drift
...
def test_drift_report_flags_prediction_and_feature_summary() -> None:
    data = _sample_data()
    ...
    report = monitor_batch_drift(data.drop(columns=["churn"]), trained_model, batch_report, config)

    assert report.row_count == len(data)
    assert "numeric" in report.feature_drift
    assert "categorical" in report.feature_drift
    assert "review_queue_size" in report.prediction_drift
    assert report.to_markdown().startswith("# Drift Monitoring Report")
